...
Generic sensor manager, abstractions and bindings of multiple sensors \href{https://github.com/kike-canaries/canairio_sensorlib/blob/master/unified-lib-deps.ini}{\texttt{ libraries}}\+: Honeywell, Plantower, Panasonic, Sensirion, etc. and CO2 sensors. Also it\textquotesingle{}s handling others environment sensors. This library is for general purpose, but also is the sensors library base of \href{https://canair.io/docs}{\texttt{ Can\+Air\+IO project}}.

For developers also you can check the complete library documentation \href{http://hpsaturn.com/canairio_sensorlib_doc/html/classSensors.html}{\texttt{ here}}
...
IKEA Vindriktning   &\PBS\centering Yes   &\PBS\centering ---   &\PBS\centering Select   &\PBS\centering STABLE    \\\cline{1-5}
...
BMP280   &\PBS\centering i2c   &\PBS\centering Auto   &\PBS\centering STABLE    \\\cline{1-4}
BME680   &\PBS\centering i2c   &\PBS\centering Auto   &\PBS\centering STABLE    \\\cline{1-4}
Df\+Robot SEN0469 NH3   &\PBS\centering i2c   &\PBS\centering Auto   &\PBS\centering TESTING    \\\cline{1-4}
DFRobot SEN0466 CO   &\PBS\centering i2c   &\PBS\centering Auto   &\PBS\centering TESTING    \\\cline{1-4}
DFRobot SEN0471 NO2   &\PBS\centering i2c   &\PBS\centering Auto   &\PBS\centering TESTING    \\\cline{1-4}
Geiger CAJOE   &\PBS\centering GPIO   &\PBS\centering Select   &\PBS\centering TESTING    \\\cline{1-4}
DHTxx   &\PBS\centering Two\+Wire   &\PBS\centering Select   &\PBS\centering DISABLED   \\\cline{1-4}
\end{longtabu}
...
ESP32\+S3   &\PBS\centering Lily\+Go TDisplay   &\PBS\centering In testing   &\PBS\centering STABLE    \\\cline{1-4}
ESP32\+C3   &\PBS\centering Devkit v3   &\PBS\centering In testing   &\PBS\centering STABLE    \\\cline{1-4}
...
\item Multiple i2c sensors and one UART sensor supported at the same time
\item Two I2C channel supported (Wire and Wire1)
\item Real time registry of sensor units (see multivariable)
...
\item Add support for Kelvin and Fahrenheit on environment and CO2 sensors
...
Full list of all sub libraries supported \href{https://github.com/kike-canaries/canairio_sensorlib/blob/master/unified-lib-deps.ini}{\texttt{ here}}
...
\DoxyCodeLine{    sensors.\mbox{\hyperlink{classSensors_aeb151247ff81fb34992458915c55ed9c}{setTemperatureUnit}}(TEMPUNIT::KELVIN);   \textcolor{comment}{// comment it for Celsius or set Fahrenheit}}
...
We recommended Platform\+IO because is more easy than Arduino IDE. For that, please install first \href{http://platformio.org/}{\texttt{ Platform\+IO}} and its command line tools (Windows, Mac\+Os and Linux), {\bfseries{pio}} command, then connect your compatible board to the USB and run the next command\+:


\begin{DoxyCode}{0}
\DoxyCodeLine{pio run -\/e esp32 -\/-\/target upload}

\end{DoxyCode}


Also you can see some examples than have {\ttfamily platformio.\+ini} files for your project.

\doxysubsubsection*{Arduino IDE}

Only import the {\ttfamily ino} file of the sample and install the libraries listed on {\ttfamily library.\+json} and this library. Complete list of libraries used \href{https://github.com/kike-canaries/canairio_sensorlib/blob/master/unified-lib-deps.ini}{\texttt{ here}}
...
\item \mbox{[}x\mbox{]} Added CO, NO2 and NH3 sensors
\item \mbox{[}x\mbox{]} Added Geiger sensor support
\item \mbox{[} \mbox{]} New IKEA VINDSTYRKA device support
